...
/**
 * @REGISTER - Registers a new user (USER or TEACHER)
 */
export const register=asyncHandler(async(req,res,next)=>{
    ...
    if(!['USER', 'TEACHER'].includes(role)){
        return next(new AppError('Invalid role. Must be USER or TEACHER', 400));
    }
    ...
    const user = await User.create({
        ...
        password,
        role,
        teacherVerification: role === 'TEACHER' ? false : undefined,
        avatar:{
            public_id:email,
    ...
    res.status(201).json({
        success:true,
        message: role === 'TEACHER' ? 'Teacher registered successfully. Awaiting admin verification.' : 'User registered successfully',
        user,
    });
});
...
/**
 * @GET_ALL_UNVERIFIED_TEACHERS - Admin retrieves all unverified teachers
 */
export const getUnverifiedTeachers = asyncHandler(async(req, res, next) => {
    try {
        ...
        res.status(200).json({
            success: true,
            message: 'Unverified teachers list',
            data: teachers,
...
/**
 * @VERIFY_TEACHER - Admin verifies a teacher
 */
export const verifyTeacher = asyncHandler(async(req, res, next) => {
    try {
        ...
        teacher.teacherVerification = true;
        await teacher.save();

        const subject = 'Teacher Account Verified';
        const message = `Congratulations! Your teacher account has been verified. You can now create courses and manage quizzes.`;
...
/**
 * @REJECT_TEACHER - Admin rejects a teacher request
 */
export const rejectTeacher = asyncHandler(async(req, res, next) => {
    try {
        ...
        const subject = 'Teacher Application Rejected';
        const message = `Your teacher application has been rejected. ${reason ? `Reason: ${reason}` : 'Please contact admin for more details.'}`;
...
/**
 * @GET_ALL_TEACHERS - Get all verified teachers
 */
